...
import { CreateTRPCSWRProxy } from '../shared/hooks/createSWRProxyHooks'
...
> = TProcedure extends AnyQueryProcedure ? {
		use: <TData = inferProcedureOutput<TProcedure>>(
			getKey: (pageIndex: number, previousPageData: TData | null) => inferProcedureInput<TProcedure> | null,
...
export type CreateTRPCInfiniteProxy<TRouter extends AnyRouter> = DecoratedProcedureRecord<TRouter['_def']['record']>

function createInfiniteProxyDecoration<TRouter extends AnyRouter>(
	name: string,
	...
	return createRecursiveProxy((opts) => {
		...
		return hooks[lastArg as keyof CreateSWRInfiniteHooks](path, geyKeyCallback, ...rest)
	})
}
...
export function createSWRInfiniteProxy<TRouter extends AnyRouter>(trpc: CreateTRPCSWRProxy<TRouter>) {
	...
	return createInfiniteProxyInternal(trpc, hooks)
}
